...
- **WHEN** `solverStrategy` is `clusterFirst` and a day's scheduled places already share a single `region`
- **THEN** the search operators never move a place carrying a different `region` onto that day, even if doing so would otherwise improve the objective
...
- **WHEN** a want- or nice-priority place remains unscheduled after the search and no day sharing its `region` has room for it
